...
export function autoLayoutNodes(nodes: Node[], edges: Edge[]):
    ...
    // their closest node
    const postit_nodes_positions: {
        node_id: string;
        closest_node_id: string;
        offset_x: number;
        offset_y: number;
    }[] = nodes
        ...
        .map((postit_node) => {
            ...
            for (const node of nodes) {
                ...
                if (node.type === 'postit_node' || node.parentId !== postit_node.parentId) continue;
                if (
                    !closest_node_id ||
    ...
    for (const subgraph of Array.from(subgraphs).reverse()) {
        ...
        const subgraph_nodes = nodes.filter(
            (node: Node) => (node.parentId ?? '') === subgraph && node.type !== 'postit_node'
        );
    ...
    nodes
        .filter((node) => node.type === 'postit_node')
        .forEach((postit_node) => {
            const position = postit_nodes_positions.find(
                (postit_node_position) => postit_node_position.node_id === postit_node.id
            )!;
