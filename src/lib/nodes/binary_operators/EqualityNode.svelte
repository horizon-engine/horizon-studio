...
<Node
	{id}
	title={getNodeTitle('equality_node')}
	{enabled}
	{error_message}
	{selected}
	handles={[
		{
			id: 'equality_node_target_1',
			type: 'target',
			y_position_percents: 30,
			valid_types: 'any',
			onconnect: (valid_types, node_handle_types) => {
				node_handle_types.set(
					'equality_node_target_2',
					coerce_types(node_handle_types.get('equality_node_target_2')!, valid_types)!
				);
			}
		},
		{
			id: 'equality_node_target_2',
			type: 'target',
			y_position_percents: 70,
			valid_types: 'any',
			onconnect: (valid_types, node_handle_types) => {
				node_handle_types.set(
					'equality_node_target_1',
					coerce_types(node_handle_types.get('equality_node_target_1')!, valid_types)!
				);
			}
		},
		{
			id: 'equality_node_source_1',
			type: 'source',
			valid_types: 'bool'
		}
	]}
	...
	<div class="my-1 space-y-3">
		<div class="flex flex-col justify-between space-y-3">
			<div class="flex-1 text-right">
				<div class="text-xs text-text-muted">{getNodeShortDescription('equality_node')}</div>
			</div>
		</div>
	</div>
</Node>
