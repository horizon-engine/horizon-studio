...
<Node
	{id}
	title={getNodeTitle('less_than_node')}
	{enabled}
	{error_message}
	{selected}
	handles={[
		{
			id: 'less_than_node_target_1',
			type: 'target',
			...
			onconnect: (valid_types, node_handle_types) => {
				node_handle_types.set(
					'less_than_node_target_2',
					coerce_types(node_handle_types.get('less_than_node_target_2')!, valid_types)!
				);
			}
		},
		{
			id: 'less_than_node_target_2',
			type: 'target',
			...
			onconnect: (valid_types, node_handle_types) => {
				node_handle_types.set(
					'less_than_node_target_1',
					coerce_types(node_handle_types.get('less_than_node_target_1')!, valid_types)!
				);
			}
		},
		{
			id: 'less_than_node_source_1',
			type: 'source',
			valid_types: 'bool'
		}
	]}
	...
	<div class="my-1 space-y-3">
		<div class="flex flex-col justify-between space-y-3">
			<div class="flex-1 text-right">
				<div class="text-xs text-text-muted">{getNodeShortDescription('less_than_node')}</div>
			</div>
		</div>
	</div>
</Node>
