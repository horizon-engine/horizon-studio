...
export function coerce_types(type0, type1) {
    if (type0 === null || type1 === null)
        return null;
    if (type0 === 'any')
        return type1 === 'none' ? null : type1;
    if (type1 === 'any')
        return type0 === 'none' ? null : type0;
    if (JSON.stringify(type0) === JSON.stringify(type1))
        return type0;
    ...
    if (types0.length <= 1 && types1.length <= 1)
        return null;
    let results = [];
...
export function getTypeColors(type) {
    switch (type) {
        case 'any':
            return ['float', 'string', 'bool', 'int'].map(t => getTypeColors(t)).flat();
        case 'int':
...
export function getConnectionValidTypes(connection) {
    const sourceType = handleValidTypes.get(connection.source)?.get(connection.sourceHandle ?? '') ?? null;
    const targetType = handleValidTypes.get(connection.target)?.get(connection.targetHandle ?? '') ?? null;
