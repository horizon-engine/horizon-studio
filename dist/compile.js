...
function get_node_input(nodes, edges, node_id, handle_id) {
...
function get_node_output(nodes, edges, node_id, handle_id) {
...
function compile_start_node(nodes, edges, node) {
...
function compile_if_node(nodes, edges, node) {
    ...
    let then_body = get_node_output(nodes, edges, node.id, 'if_node_source_1');
    then_body = then_body ? compile_node(nodes, edges, then_body) : [];
    let else_body = get_node_output(nodes, edges, node.id, 'if_node_source_2');
    else_body = else_body ? compile_node(nodes, edges, else_body) : [];
    let finally_body = get_node_output(nodes, edges, node.id, 'if_node_source_3');
...
function compile_while_node(nodes, edges, node) {
    let condition = get_node_input(nodes, edges, node.id, 'while_node_target_2');
    if (condition === null) {
        throw new Error('a condition is required', { cause: node.id });
    }
    condition = compile_node(nodes, edges, condition);
    let body = get_node_output(nodes, edges, node.id, 'while_node_source_1');
    body = body ? compile_node(nodes, edges, body) : [];
    let finally_body = get_node_output(nodes, edges, node.id, 'while_node_source_2');
    finally_body = finally_body ? compile_node(nodes, edges, finally_body) : [];
    return [
        {
            source_map: node.id,
...
function compile_set_variable_node(nodes, edges, node) {
    ...
    let finally_body = get_node_output(nodes, edges, node.id, 'set_variable_node_source_1');
    finally_body = finally_body ? compile_node(nodes, edges, finally_body) : [];
    return [
        {
            source_map: node.id,
...
function compile_value_node(node) {
    ...
    function generate_value_object(type, value) {
        if (type === 'int') {
            return {
    ...
    return {
        source_map: node.id,
        value: {
            Value: generate_value_object(type, value)
        }
    };
}
function compile_variable_node(node) {
    return {
        source_map: node.id,
        value: {
            Variable: node.data.variable.name
        }
    };
}
function compile_not_node(nodes, edges, node) {
...
function compile_binary_operator_node(nodes, edges, node) {
...
function compile_node(nodes, edges, node) {
    switch (node.type) {
        case 'start_node':
...
export function compile(variables, nodes, edges) {
    return {
